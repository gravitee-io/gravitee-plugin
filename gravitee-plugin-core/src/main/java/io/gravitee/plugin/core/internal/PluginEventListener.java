            ...
            if (pos1 >= 0) {
                if (pos2 >= 0) {
                    if (pos1 == pos2) {
                        if (
                            loadFirst != null &&
                            o1.manifest().type().equalsIgnoreCase(SECRET_PROVIDER) &&
                            o1.manifest().id().equals(loadFirst)
                        ) {
                            return -1;
                        } else if (
                            loadFirst != null &&
                            o2.manifest().type().equalsIgnoreCase(SECRET_PROVIDER) &&
                            o2.manifest().id().equals(loadFirst)
                        ) {
                            return 1;
                        }
                        return 0;
